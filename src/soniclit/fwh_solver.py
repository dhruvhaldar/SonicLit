...
def calculate_source_terms_serial(surf_file : str, preprocessed_data, ambient_pressure, ambient_density, speed_of_sound, mach_number, f, is_permeable, skip_Qn=False):
    ...
    if isinstance(preprocessed_data, dict):
        ...
        geom_r = preprocessed_data.get('r') # Optional
    else:
        # Legacy path
    ...
    return {'Qn': Qn, 'Lm': Lm, 'Lr': Lr, 'L_vec': (L1, L2, L3)}
...
def stationary_serial(surf_file : str,  output_filename : str, observer_locations : list, source_times : list, mach_number : list, is_permeable : bool, write : bool = True, ambient_temperature : float = 298, gamma : float = 1.4, MW : float = 28.97):
        ...
        # Optimization: Batch processing for observers to balance I/O reduction with memory usage
        # This significantly reduces file reads when multiple observers are present.
        BATCH_SIZE = 10
        num_observers = len(observer_locations)

        for batch_start in range(0, num_observers, BATCH_SIZE):
            batch_end = min(batch_start + BATCH_SIZE, num_observers)
            current_observers = observer_locations[batch_start:batch_end]

            # Initialize storage and precompute parameters for this batch
            batch_params = []

            for idx, xo in enumerate(current_observers):
                ...
                # count = np.zeros(len(t_o)+D) # count is accumulating n_elm but not used for normalization in final output here?
                # Preserving original logic: count was updated but unused in plot/csv output in this function scope.
                ...
                params = {
                    ...
                    'factors_scaled': (
                        factor_pt1 * inv_4pi,
                        factor_pt2 * (speed_of_sound * inv_4pi),
                        factor_pq1 * inv_4pi,
                        factor_pq2 * inv_4pi,
                        factor_pq3 * inv_4pi
                    ),
                    'pt_static': None,
                    'geom_r': r_vec,
                    'Lrdot_next': None, # State for differentiation
                    'Qndot_next': None
                }
                ...
                if not is_permeable:
                    ...
                    params['pt_static'] = Qn_static * params['factors_scaled'][1]

                batch_params.append(params)

            # Precompute/Preload initial 3 time steps
            # Pass only geom_n to avoid computing Lr (observer dependent)
            preprocessed_arrays_no_r = {'n': geom_n} # geom_r is optional now

            if is_permeable:
                U0_dot_n_static = np.dot(geom_n, U0_static)
                preprocessed_arrays_no_r['U0_dot_n'] = U0_dot_n_static
                skip_Qn = False
            else:
                skip_Qn = True # Qn is static/precomputed

            # Buffer to store L_vec, Qn, Lm from last 4 steps
            # src_buffer = [step0, step1, step2, step3]
            src_buffer = []

            for i in range(3):
                src = calculate_source_terms_serial(surf_file+str(i)+'.csv', preprocessed_arrays_no_r, ambient_pressure, ambient_density, speed_of_sound, mach_number, filt, is_permeable, skip_Qn=skip_Qn)
                src_buffer.append(src)

            # Main Time Loop
            for j in range(1, len(source_times)-2):
                # Read next step
                src_next = calculate_source_terms_serial(surf_file+str(j+2)+'.csv', preprocessed_arrays_no_r, ambient_pressure, ambient_density, speed_of_sound, mach_number, filt, is_permeable, skip_Qn=skip_Qn)
                src_buffer.append(src_next)

                # Extract buffered data for ease of access
                src0 = src_buffer[0]
                src1 = src_buffer[1]
                src2 = src_buffer[2]
                src3 = src_buffer[3]
                ...
                # Common data
                Lm0, Lm1, Lm2, Lm3 = src0['Lm'], src1['Lm'], src2['Lm'], src3['Lm']

                if is_permeable:
                    Qn0, Qn1, Qn2, Qn3 = src0['Qn'], src1['Qn'], src2['Qn'], src3['Qn']
                ...
                # Loop over observers in this batch
                for params in batch_params:
                    # Unpack params
                    j_star = params['j_star']
                    len_p_act = params['len_p_act']
                    interpolation_weight = params['interpolation_weight']
                    sp_c0, sp_c1, sp_c2, sp_c3 = params['sp_coeffs']
                    factor_pt1_s, factor_pt2_s, factor_pq1_s, factor_pq2_s, factor_pq3_s = params['factors_scaled']
                    geom_r = params['geom_r']

                    # Compute Lr for this observer
                    # Lr = L1*rx + L2*ry + L3*rz
                    Lr0 = L_vec0[0]*geom_r[:,0] + L_vec0[1]*geom_r[:,1] + L_vec0[2]*geom_r[:,2]
                    Lr1 = L_vec1[0]*geom_r[:,0] + L_vec1[1]*geom_r[:,1] + L_vec1[2]*geom_r[:,2]
                    Lr2 = L_vec2[0]*geom_r[:,0] + L_vec2[1]*geom_r[:,1] + L_vec2[2]*geom_r[:,2]
                    Lr3 = L_vec3[0]*geom_r[:,0] + L_vec3[1]*geom_r[:,1] + L_vec3[2]*geom_r[:,2]

                    # Lrdot
                    if params['Lrdot_next'] is None:
                        Lrdot1 = (-Lr0+Lr2)*inv_2dt
                    else:
                        Lrdot1 = params['Lrdot_next']

                    Lrdot2 = (-Lr1+Lr3)*inv_2dt
                    params['Lrdot_next'] = Lrdot2
                    ...
                    if not is_permeable:
                        ...
                        if params['Qndot_next'] is None:
                            Qndot1 = (-Qn0+Qn2)*inv_2dt
                        else:
                            Qndot1 = params['Qndot_next']

                        Qndot2 = (-Qn1+Qn3)*inv_2dt
                        params['Qndot_next'] = Qndot2

                        Qn = sp_c0*Qn0 + sp_c1*Qn1 + sp_c2*Qn2 + sp_c3*Qn3
                        Qndot = Qndot2 + interpolation_weight * (Qndot1 - Qndot2)

                        pt = Qndot * factor_pt1_s + Qn * factor_pt2_s

                    # Loading component
                    pq = Lrdot * factor_pq1_s + (Lr - Lm) * factor_pq2_s + Lr * factor_pq3_s
                    p = pt+pq

                    j_adv = j + j_star + 1
                    j_cond = (j_adv >= params['t_range'][0])*(j_adv < params['t_range'][1])
                    p *= j_cond

                    p_act = np.bincount(j_star, weights=p, minlength=len_p_act)
                    # count update skipped as it was unused for output

                    start_idx = min(j_adv)
                    end_idx = max(j_adv) + 1
                    params['acoustic_pressure'][start_idx:end_idx] += p_act
            ...
            for params in batch_params:
                ...
                acoustic_pressure = params['acoustic_pressure']
                ...
                ax.set_ylabel("$p'$ (Pa)")
                fig.savefig(output_filename + str(idx)+ '.png')
                plt.close(fig)
                if write :
