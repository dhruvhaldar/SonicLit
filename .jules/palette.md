...
## 2026-06-25 - Grouping Sequential Inputs in Tkinter Forms\n**Learning:** Vertically stacking related configuration parameters (like 'Time Step' and 'Number of Steps') in Tkinter desktop applications wastes vertical space and creates visual clutter. Streamlit addresses this inherently with , but in Tkinter, each new label/entry pair placed on a new grid row exacerbates the issue.\n**Action:** When refactoring or building Tkinter data entry forms, encapsulate logically paired or sequential inputs (like X/Y/Z components or time configurations) inside a dedicated inner . Use the inner frame's grid to lay them out horizontally (e.g., ) on a single parent row. This mirrors the  UX pattern, drastically improving form density and user cognitive alignment.

## 2026-06-25 - Grouping Sequential Inputs in Tkinter Forms
**Learning:** Vertically stacking related configuration parameters (like 'Time Step' and 'Number of Steps') in Tkinter desktop applications wastes vertical space and creates visual clutter. Streamlit addresses this inherently with `st.columns`, but in Tkinter, each new label/entry pair placed on a new grid row exacerbates the issue.
**Action:** When refactoring or building Tkinter data entry forms, encapsulate logically paired or sequential inputs (like X/Y/Z components or time configurations) inside a dedicated inner `ttk.Frame`. Use the inner frame's grid to lay them out horizontally (e.g., `row=0, col=0,1,2,3`) on a single parent row. This mirrors the `st.columns` UX pattern, drastically improving form density and user cognitive alignment.

## 2026-06-12 - Usable Float Steps in Streamlit Number Inputs
**Learning:** In Streamlit, `st.number_input` widgets for floating-point values implicitly default to a step size of `0.01`. For inputs representing very small physical quantities (like Time Step around 0.01 s) or precise formatted decimals (`%.4f`), this default makes the widget's native `+` and `-` spin buttons practically useless or frustrating, as adjusting the value skips meaningful intermediate decimal places.
**Action:** Always evaluate the physical domain of a floating-point `st.number_input`. Explicitly define a contextually meaningful `step` parameter (e.g., `step=0.001` or `step=1.0`) to ensure the increment/decrement interactions are intuitive and helpful rather than tedious or unaligned with the expected input format.
## 2026-06-25 - Avoid Contradictory File Size Limits in Streamlit
**Learning:** Appending custom file size limits (like "Max 50MB") to the label or help text of an `st.file_uploader` creates a jarring UX contradiction, because Streamlit natively hardcodes and displays its own generic limit ("Limit 200MB per file") directly inside the drag-and-drop zone.
**Action:** Never try to override or append file size limits to `st.file_uploader` labels. If backend limits differ from Streamlit's default, surface this information elsewhere (like a separate info banner or validation error) rather than creating conflicting inline instructions.
